...
class CPU:
    ...
    def run(self):
        ...
        while running:
            ...
            # elif command == CALL:
            #     # 2. The PC is set to the address stored in the given register. Jump to that location in RAM and execute the first instruction in the subroutine. The PC can move forward or backwards from its current location.
            ...
            #     self.pc += 2
                ...
                print("Compare the values in two registers")
                self.alu("CMP", operand_a, operand_b)
                self.pc += 3

            elif command == JMP:
                print("Jump to the address stored in the given register")
                self.pc = self.reg[operand_a]

            elif command == JEQ:
                print("If `equal` flag is set (true), jump to the address stored in the given register")
                if self.e_flag == 1:
                    self.pc = self.reg[operand_a]
                else:
                    self.pc += 2
